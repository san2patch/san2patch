...
from san2patch.dataset.reproducer.reproducer import VulnlocReproducer
from san2patch.utils.enum import LanguageEnum


class VulnLocDataset(BaseDataset):
    # name = "final-test"
    name = "vulnloc"
    language = LanguageEnum.C

    def chk_download(self):
        return os.path.exists(os.path.join(self.download_dir, "vulnloc-benchmark"))
    ...
    def download(self):
        ...
        self.run_cmd(
            f"git clone https://github.com/nus-apr/vulnloc-benchmark/",
    ...
    def extract(self):
        ...
        metadata_file = os.path.join(self.download_dir, "vulnloc-benchmark", "meta-data.json")
        ...
        for vuln_data in metadata:
            ...
            if os.path.exists(os.path.join(self.san_dir, f"{vuln_data['bug_id']}.san")) or os.path.exists(
                ...
                reproducer = VulnlocReproducer(vuln_data)
                reproducer.run()
        ...
        for root, _, files in os.walk(os.path.join(self.extracted_dir, "vuln")):
            for file in files:
                ...
                try:
                    ...
                    # data_dir = os.path.join(self.download_dir, 'vulnloc-benchmark', vuln_data["subject"], vuln_id)
