...
class get_code_by_file_with_lines(BaseModel):
    ...
    file_name: str = Field(..., description="The name of the file to search for code snippet")
    line_start: int = Field(..., description="The starting line number of the code snippet")
...
class get_code_block_from_file_with_lines(BaseModel):
    ...
    file_name: str = Field(..., description="The name of the file to search for code block")
...
class get_func_body_from_file_with_lines(BaseModel):
    ...
    file_name: str = Field(..., description="The name of the file to search for code block")
    line_start: int = Field(..., description="The starting line number of the code block")
...
class ContextManager:
    def __init__(self, language: Literal["C"], src_dir, context_mode: CODE_CONTEXT_MODE = "ast"):
    ...
    def get_file_content(self, file_uri: str) -> str:
        ...
        self.logger.debug(f"Getting content of {file_path} from line {start_line} to {end_line}")
    ...
    def get_function_def_and_ret(self, file_name, target_line) -> Tuple[str, list[str]]:
                            ...
                            def extract_returns(statement_node):
                                if statement_node.type == "return_statement":
                                    return_statements.append(
                                        code[statement_node.start_byte : statement_node.end_byte].strip()
                                    )
        ...
        # assert len(functions) <= 1, "Multiple functions found in the target line"

        if len(functions) == 0:
            raise ValueError(f"Function not found in the file {file_name} at line {target_line}")
    ...
    def get_func_body(self, file_name: str, line_start: int, line_end: int) -> str:
        ...
        for _, function_line in function_infos.items():
            ...
            if function_line[0] <= line_start <= function_line[1] and function_line[0] <= line_end <= function_line[1]:
                res_line = function_line
                break
            # If the function line is within the line_start and line_end
            if line_start <= function_line[0] <= line_end and line_start <= function_line[1] <= line_end:
                res_line = function_line
                break
        ...
        if res_line:
            file_uri = f"file://{self.src_dir}/{file_name}:{res_line[0]}:1:{res_line[1]}:1"
        else:
            self.logger.error(f"Function not found between lines {line_start} and {line_end} in the file.")
    ...
    @traceable(run_type="tool")
    def get_backward_code_block(self, file_name: str, line: int, min_line: int = 20, max_line: int = 50) -> str:
        code_line = self.get_code_lines(file_name, line, line)
        original_code = self.get_code_block(file_name, line, min_line * 2, max_line * 2).splitlines()

        return "\n".join(original_code[: original_code.index(code_line.replace("\n", "")) + 1])
    ...
    def get_ast_node(self, file_name, line):
        ...
        def find_node_containing_line(node, line_start):
            if node.start_point[0] == line_start - 1 and line_start - 1 == node.end_point[0]:
                return node
            else:
                for child in node.children:
        ...
        if node == None:
            self.logger.error(f"Cannot find the ast node that contains line {line} in the file {file_name}")
    ...
    def get_code_context(
        self, file_name: str, line: int, min_line: int = 20, max_line: int = 50, sibling: bool = True
    ) -> CodeContextResponse:
        if self.context_mode == "line":
            avg_line = (min_line + max_line) // 2
            return CodeContextResponse(
                code=self.get_code_lines(file_name, max(1, line - avg_line // 2), line + avg_line // 2).strip(),
            )

        else:
            try:
                code_block = self.get_code_block(file_name, line, min_line, max_line, sibling)
                try:
                ...
                avg_line = (min_line + max_line) // 2
                cod_block = (
                    self.get_code_lines(file_name, max(1, line - avg_line // 2), line + avg_line // 2).strip(),
                )

            return CodeContextResponse(code=code_block, func_def=func_def, func_ret=func_ret)
    ...
    def get_code_block(
        ...
        def find_enclosing_blocks(node, levels):
            ...
            while node is not None and levels > 0:
                if node.type == "compound_statement":  # This is for blocks enclosed in {}
        ...
        def get_code_block_str2(node, source_code):
            ...
            extracted_code = get_code_str_by_points(node.start_point, node.end_point, source_code)
            if extracted_code != node.text.decode("utf-8"):
                self.logger.warning("Code block extracted using points doesn't match the node text")
        ...
        if code_block and len(code_block.split("\n")) <= max_line and len(code_block.split("\n")) >= min_line:
            ...
            if node.parent:
                ...
                if parent_prev and parent_next:
                    ...
                    code_str = get_code_str_by_points(parent_prev.start_point, parent_next.end_point, source_code)
                    if len(code_str.split("\n")) <= max_line:
                        return code_str

                if parent_prev:
                    # code_str = get_code_str_by_bytes(parent_prev.start_byte, parent.end_byte, source_code)
                    code_str = get_code_str_by_points(parent_prev.start_point, parent.end_point, source_code)
                    if len(code_str.split("\n")) <= max_line:
                        return code_str

                if parent_next:
                    # code_str = get_code_str_by_bytes(parent.start_byte, parent_next.end_byte, source_code)
                    code_str = get_code_str_by_points(parent.start_point, parent_next.end_point, source_code)
                    if len(code_str.split("\n")) <= max_line:
                        return code_str
            ...
            if node_prev and node_next:
                ...
                code_str = get_code_str_by_points(node_prev.start_point, node_next.end_point, source_code)
                if len(code_str.split("\n")) <= max_line:
                    return code_str

            if node_prev:
                # code_str = get_code_str_by_bytes(node_prev.start_byte, node.end_byte, source_code)
                code_str = get_code_str_by_points(node_prev.start_point, node.end_point, source_code)
                if len(code_str.split("\n")) <= max_line:
                    return code_str

            if node_next:
                # code_str = get_code_str_by_bytes(node.start_byte, node_next.end_byte, source_code)
                code_str = get_code_str_by_points(node.start_point, node_next.end_point, source_code)
                if len(code_str.split("\n")) <= max_line:
                    return code_str
    ...
    def get_func_body_from_code(self, code: str, function_name: str) -> str:
        self.logger.debug(f"Searching for function body of {function_name} in code string")
        ...
        # Extract function names
        try:
            function_body = get_function_body(tree.root_node, bytes(code, "utf8"), function_name)
        except ValueError as e:
